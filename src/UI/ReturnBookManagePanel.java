...
public class ReturnBookManagePanel  extends JPanel{
	...
	private ReturnBookModel bookTableModel = null;
	private TablePanel<BorrowBook> TablePanel = null;
	private UpdateReturnBookPanel newbookPanel = null;
	private UpdateReturnBookPanel UpdateReturnBookPanel = null;
	/*----------底部按钮------------*/
	/** 新增按钮 */
	...
	private ReturnBookService bookService = null;
	 private Account administrator = null;
	@SuppressWarnings("javadoc")
	public ReturnBookManagePanel(Account administrator) {
		...
		this.bookTableModel = new ReturnBookModel();
		this.bookService = (ReturnBookService) ServiceFactory.newInstanceService("ReturnBookService");;
		...
		column.setPreferredWidth(0);
		 this.table.getColumn("归还时间").setPreferredWidth(200);
		...
		this.newbookPanel = new UpdateReturnBookPanel(administrator,null);
		initTabbedPane();
		this.add(tabbedPane);
	}
	...
	private JPanel createUpdaPanel(BorrowBook book) {
		...
		this.UpdateReturnBookPanel = new UpdateReturnBookPanel(administrator,book);
		panel.add(UpdateReturnBookPanel);
		return panel;
	}
...
//		btnPanel.setBorder(BorderFactory.createLineBorder(Color.red,2));
	...
	private void addBtnEvent() {
		this.btn_add.addActionListener(e -> {
			...
			// 更新数据
			ReturnBookManagePanel.this.TablePanel.renderTableData(0, TablePanel.getPageSize());
		});
		this.btn_update.addActionListener(e -> {
			JTable bookTable = ReturnBookManagePanel.this.TablePanel.getTable();
			// 获取选中行
			int[] selectedRows = bookTable.getSelectedRows();
			if (selectedRows == null || selectedRows.length == 0) {
				JOptionPane.showMessageDialog(this, "您尚未选择要修改的还书记录！");
				return;
			} else if (selectedRows.length > 1) {
				JOptionPane.showMessageDialog(this, "请选择一本要修改的还书记录！");
				return;
			} else {
				// 获取选中行
				...
				JPanel UpdateReturnBookPanel = createUpdaPanel(borrowBook);
				tabbedPane.addTab("修改", UpdateReturnBookPanel);
				tabbedPane.setSelectedComponent(UpdateReturnBookPanel);
				// 更新数据
				ReturnBookManagePanel.this.TablePanel.renderTableData(0, TablePanel.getPageSize());
			}
		});
	...
	private void outPutDataBtnEvents(JButton btn) {
		btn.addActionListener(e -> {
			JTable bookTable = ReturnBookManagePanel.this.TablePanel.getTable();
			BaseTableModel<BorrowBook> tableModel = ReturnBookManagePanel.this.TablePanel.getTableModel();
			if ("导出当选中数据".equals(btn.getText())) {
				...
				int allRowCount = ReturnBookManagePanel.this.TablePanel.getAllRowCount();
				if (allRowCount > 0) {
					// 将bookTable的所有数据都渲染出来
					ReturnBookManagePanel.this.TablePanel.renderTableData(0, allRowCount);
				}
