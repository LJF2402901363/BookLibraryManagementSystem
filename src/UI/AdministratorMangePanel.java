 ...
 * @author 陌意随影
 TODO :管理员管理界面
 *2020年3月18日  上午11:58:14
 */
public class AdministratorMangePanel extends JPanel {
	private AdministratortService administratortService = null;
	...
	public AdministratorMangePanel(Account administrator) {
...
//		btnPanel.setBorder(BorderFactory.createLineBorder(Color.red,2));
	...
	private void addBtnEvent() {
		this.btn_add.addActionListener(e -> {
			...
			// 更新数据
			AdministratorMangePanel.this.accountTablePanel.renderTableData(0, accountTablePanel.getPageSize());
		});
		this.btn_update.addActionListener(e -> {
			JTable AccountTable = AdministratorMangePanel.this.accountTablePanel.getTable();
			// 获取选中行
			...
			if (selectedRows == null || selectedRows.length == 0) {
				...
				Account Account = administratortService.findAccountById(id);
				for (int i = 0; i < tabbedPane.getTabCount(); i++) {
					String titleAt = tabbedPane.getTitleAt(i);
					// 该标签页已经存在
		...
		this.btn_remove.addActionListener(e -> {
			...
			if (selectedRows == null || selectedRows.length == 0) {
				...
				for (int i = 0; i < selectedRows.length; i++) {
					...
					boolean fla = administratortService.removeAccountById(id);
					if(fla) {
	...
	private void outPutDataBtnEvents(JButton btn) {
		btn.addActionListener(e -> {
			JTable AccountTable = AdministratorMangePanel.this.accountTablePanel.getTable();
			BaseTableModel<Account> tableModel = AdministratorMangePanel.this.accountTablePanel.getTableModel();
			...
			if ("导出当选中数据".equals(btn.getText())) {
				...
				int allRowCount = AdministratorMangePanel.this.accountTablePanel.getAllRowCount();
				if (allRowCount > 0) {
					// 将AccountTable的所有数据都渲染出来
					AdministratorMangePanel.this.accountTablePanel.renderTableData(0, allRowCount);
				}
